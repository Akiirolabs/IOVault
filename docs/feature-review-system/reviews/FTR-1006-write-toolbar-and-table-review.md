...
| Recorded | 2026-07-29 |
| Status | **Verified — 14 of 14 findings completed** |
...
| &emsp;&emsp;↳ `FTR-1006.4.1` | External-page paste should remove copied backgrounds and unsafe source markup while preserving useful text formatting | Verified |
...
| &emsp;&emsp;&emsp;&emsp;↳ `FTR-1006.6.2.3.1` | Formula cells should expose configuration when incomplete and explain the required named-column syntax | Verified |
...
The Write editor now reveals its unified formatting group only over the dedicated control area, normalizes external paste without copied backgrounds or unsafe source markup, keeps linked-page controls sticky, minimizes overlays on outside-click, removes the unused action column, and moves Enter to the same property in the next visible row. Column menus expose editing and explicit sort direction. Collections support Currency, Percent, Email, Single select, Formula, Relation, and embedded Page properties; incomplete Formula cells open their configuration directly and explain named-column references, formulas evaluate bounded arithmetic, relations link existing Write pages, and Page content remains owned by its collection cell.
...
| Gate | Result |
|---|---|
| Focused model and component coverage | Passed — 30 tests |
