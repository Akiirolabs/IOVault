...
export function ProjectTableEditor({ table, onChange }: { table: ProjectTable; onChange: (table: ProjectTable) => void }) {
  ...
  return <section className="project-mode-surface project-table-mode" aria-label="Project table">
    <header><h3>Table</h3><div><button type="button" onClick={() => onChange({ ...table, rows: [...table.rows, { id: id("row"), cells: {} }] })}>+ Row</button><button type="button" aria-expanded={isAddingColumn} onClick={() => setIsAddingColumn((value) => !value)}>+ Column</button></div></header>
