...
export function reorderProjects(blocks: ProjectBlock[], projectId: string, targetId: string) {
...
export function normalizeProjectBlock(raw: unknown): ProjectBlock | null {
  ...
  const flowchart = value.flowchart && typeof value.flowchart === "object" && Array.isArray(value.flowchart.nodes) && Array.isArray(value.flowchart.edges) ? {
    nodes: value.flowchart.nodes.flatMap((node) => node && typeof node === "object" && typeof node.id === "string" && typeof node.label === "string" ? [{ id: node.id, label: node.label, x: Number.isFinite(node.x) ? Math.max(0, Number(node.x)) : 40, y: Number.isFinite(node.y) ? Math.max(0, Number(node.y)) : 40, color: typeof node.color === "string" ? node.color : "#38bdf8" }] : []),
